# Team Charter

## Part 1: Team Members

**Project Name:** Care Path

**Team Members:**

- Michelle Borkiano
- Desmond Balentine
- Remington Mustadter

**Team Vision/Purpose (Reason for the team. What is/are the group goal(s)?):**
Build a transportation coordination tool that helps people access care reliably and on time.

**Results/Deliverables (We will be successful when...):**
The platform is completed, functional, and deliverable as a usable product.

**Members Skills/Knowledge/Expertise (How will you capitalize on the strengths of each member?):**

- Desmond: Backend development, architecture, leadership
- Michelle: Frontend, design, communication
- Remington: AI support, coding, troubleshooting

**Team Member Roles (How will you assign group roles? What are the strengths and weaknesses of each team member?):**

- Desmond: Project lead and product direction; assigns project targets and tracks milestones.
- Michelle: Design lead and communication support; handles UX, visuals, and presentation flow.
- Remington: Technical troubleshooting lead; supports AI-enabled features and cross-team problem solving.

---

## Team Member Work Profile Table

| Name | Skills/Strengths | Group Skills to Work On | Stressors | Group Role(s) |
| --- | --- | --- | --- | --- |
| Michelle Borkiano | Research, communication, design | Prioritization under pressure | Too many large tasks at once | Design, communication |
| Desmond Balentine | Architecture, communication, backend leadership | Delegation and schedule balancing | Missed deadlines | Product design, project lead |
| Remington Mustadter | Coding, troubleshooting, leadership, teamwork | Time estimation and pacing | Missed deadlines | Floater/support role to keep project grounded and unblocked |

---

## Part 2: Team Processes

### 1. Communication

**How the team will communicate (frequency and methods):**

- Zoom, text, and email
- Quick check-ins 2-3 times per week
- Weekly progress sync meeting

**Maximum expected response time:**

- 24 hours for normal messages
- Same day for urgent blocker messages

### 2. Decision-Making

**How decisions will be made in this team / how you will stay on track:**

- Case-by-case review with team input
- Final project-level calls made by Desmond after team suggestions
- Set deadlines, track progress weekly, and lock milestones

### 3. Conflict Resolution

**How will you resolve differences?**

- Talk issues through directly and respectfully
- Revisit shared goals and role commitments
- Escalate unresolved items to a full-team discussion

### 4. Commitments

**How will you handle different levels of participation and commitment?**

- Work with each other and actively support teammates
- Raise concerns early when workload or timelines are at risk
- Reassign tasks if needed to keep delivery on track

**Process if someone does not meet responsibilities:**

1. Private check-in to understand the blocker
2. Agree on a recovery plan and new deadline
3. Rebalance tasks if needed

**Consequences for poor performance:**

- Reduced ownership on critical-path tasks
- Additional check-ins and progress accountability

### 5. Other

**Additional commitments/goals/processes:**

- Keep communicating consistently
- Maintain shared visibility of deadlines and responsibilities
- Support each other to complete the platform successfully

---

## Signatures

- Member 1 Printed Name: Michelle Borkiano
- Member 1 Signature: ______________________________
- Date: 07/13/2026

- Member 2 Printed Name: Remington Mustadter
- Member 2 Signature: ______________________________
- Date: 07/13/2026

- Member 3 Printed Name: Desmond Balentine
- Member 3 Signature: ______________________________
- Date: 07/15/2026

- Member 4 Printed Name: ___________________________
- Member 4 Signature: ______________________________
- Date: __________

- Member 5 Printed Name: ___________________________
- Member 5 Signature: ______________________________
- Date: __________
